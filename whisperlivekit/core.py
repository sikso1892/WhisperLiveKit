    ...
    def _do_init(self, config=None, **kwargs):
        ...
        if config.transcription:
            if config.backend == "vllm-realtime":
                ...
                self.asr.backend_choice = "granite-speech"
                logger.info("Using Granite 4.0 1B Speech backend (batch, English)")
            elif config.backend == "granite-speech-vllm":
                from whisperlivekit.granite_vllm_asr import GraniteVLLMASR
                self.asr = GraniteVLLMASR(
                    **transcription_common_params,
                    gpu_memory_utilization=config.gpu_memory_utilization,
                )
                self.asr.confidence_validation = config.confidence_validation
